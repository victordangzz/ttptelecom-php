...
if($id!='') {
    /* Lấy bài viết detail */
	$rowDetail = $d->rawQueryOne("select id, view, date_created, id_list, id_cat, id_item, id_sub, type, name$lang, slug$lang, desc$lang, content$lang, photo, options from #_news where id = ? and type = ? and find_in_set('hienthi',status) limit 0,1",array($id,$type));
    ...
    $tags = $d->rawQuery("select name$lang, id from #_tags where type = ? and find_in_set('hienthi',status) order by numb,id desc", array($type));
    $news = $d->rawQuery("select name$lang, desc$lang from #_news where type = ? and find_in_set('hienthi',status) order by numb,id desc", array($type));
    if (!empty($titleMain)) $breadcr->set($com, $titleMain);
    $breadcrumbs = $breadcr->get();
}
