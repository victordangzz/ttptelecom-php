$.fn.comments = function (options) {
	...
	var mediaSlid = function () {
		wrapperLists.find(".carousel-comment-media").each(function () {
			...
			$this.on("slid.bs.carousel", function (e) {
				$thisSlid = $(this);
				var videoActive = $thisSlid.find(
					".carousel-lists .carousel-comment-media-item-video.active"
				);
				var videoItem = $thisSlid.find(
					".carousel-lists .carousel-comment-media-item-video"
				);
	...
	var mediaVideo = function () {
		if (isExist($("#review-poster-video"))) {
			photoZone(
				"#review-poster-video-label",
				"#review-poster-video",
				"#review-poster-video-preview img",
				""
			);
		}
	};
	...
	wrapper.on(
		...
		function (e) {
			...
			var id = $this.data("id");
			var videoThis = $parents.find(
				".carousel-lists .carousel-comment-media-item-" + id
			);
			var videoItem = $parents.find(
				".carousel-lists .carousel-comment-media-item-video"
			);

			if ($this.hasClass("active")) {
				$parents
					.find(".carousel-indicators li, .carousel-lists .carousel-item")
					.removeClass("active");
				videoItem.find("#file-video").trigger("pause");
			} else {
